...
def create_celeba_hq(h5_filename, celeba_dir, delta_dir, save_dir, num_threads=4, num_tasks=100):
    ...
    def process_func(idx):
        ...
        kdf = cryptography.hazmat.primitives.kdf.pbkdf2.PBKDF2HMAC(algorithm=algorithm, length=32, salt=orig_file.encode(), iterations=100000, backend=backend)
        ...
        # 修改为返回去掉了.jpg的文件名，并且返回图像
        orig_file = orig_file.replace('.jpg', '.png')
    ...
    with ThreadPool(num_threads) as pool:
        for orig_fn, aimg64, aimg128, aimg256, aimg512, aimg1024 in pool.process_items_concurrently(fields['idx'], process_func=process_func, max_items_in_flight=num_tasks):
            ensure_dir(save_dir + '/celeba-256/')
            ensure_dir(save_dir + '/celeba-512/')
            ensure_dir(save_dir + '/celeba-1024/')
            # 使用PNG格式保存，并且保持与img_celeba中的文件名一致
            # aimg64.save('./celeba-hq/celeba-64/' + str(orig_fn), format='PNG')
            # aimg128.save('./celeba-hq/celeba-128/' + str(orig_fn), format='PNG')

            aimg256.save(save_dir + '/celeba-256/' + str(orig_fn), format='PNG')
            aimg512.save(save_dir + '/celeba-512/' + str(orig_fn), format='PNG')
            aimg1024.save(save_dir + '/celeba-1024/' + str(orig_fn), format='PNG')
            print(orig_fn)
#----------------------------------------------------------------------------
...
def main():
    ...
    p.add_argument(     '-celeba_dir',       help='Directory to read CelebA data from', default='')
    p.add_argument(     '-delta_dir',        help='Directory to read CelebA-HQ deltas from', default='')
    p.add_argument('-save_dir', help='where', default='')
    ...
    args = p.parse_args()
    if args.create_what == 'celebahq':
        create_celeba_hq(args.h5_filename, args.celeba_dir, args.delta_dir, args.save_dir, args.num_threads, args.num_tasks)
